...
import { FaStar, FaHeart, FaChevronDown, FaTimes, FaCamera, FaSpa, FaUser, FaEnvelope, FaPhone, FaCalendarAlt, FaUsers, FaChild, FaHotel, FaUtensils, FaWallet, FaPlus, FaMinus, FaExclamationCircle } from 'react-icons/fa';
  ...
  const paymentHandler = async (e) => {
    ...
    try {
      ...
      const options = {
        ...
        currency: currency,
        name: "Traveligo",
        description: `Kashmir Tour Package - ${selectedPackage.title}`,
        ...
        notes: {
          ...
          destination: "Kashmir",
          booking_reference: `KASHMIR-${Date.now()}`,
  ...
  const packages = [
    ...
    },
    {
      ...
      highlights: [
        ...
        "Kashmiri handicraft workshops",
        "Traditional music performance",
        "Local cuisine cooking class",
        "Heritage walk in old city",
        "Pink-themed cultural evening"
      ],
      icon: <GiFlowerPot className="text-2xl text-pink-500" />,
      pinkFeature: "Pink Kashmiri handicraft souvenir"
    },
    {
      id: 8,
      ...
      price: "₹19,999",
      rating: 4.8,
      image: "/images/kashmir2.webp",
      type: "spiritual",
      highlights: [
        "Meditation sessions by Dal Lake",
        "Yoga at sunrise",
        "Visit to sacred shrines",
        "Ayurvedic wellness treatments",
        "Nature walks in pine forests",
        "Pink lotus meditation session"
      ],
      icon: <IoLeaf className="text-2xl text-pink-500" />,
      pinkFeature: "Pink meditation mats and cushions"
    }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-pink-50">
      ...
      <div className="relative z-10 py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto -mt-20">
        ...
        <motion.div 
          ...
          viewport={{ once: true }}
        >
          {['all', 'popular', 'honeymoon', 'winter', 'family', 'adventure', 'luxury', 'cultural', 'spiritual'].map((tab) => (
            <motion.button
              key={tab}
                  ...
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <motion.button
                        ...
                        className={`w-full bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white py-3 rounded-lg font-bold transition-all duration-200 shadow-lg ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
